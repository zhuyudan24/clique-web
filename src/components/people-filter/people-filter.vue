          ...
          <el-form label-position="right" label-width="90px" :model="ruleForm" :rules="rules" ref="ruleForm">
            <el-form-item label="会员等级" class="form-item">
              <div class="group-value">
                <template>
                  <el-select v-model="gradeLevel" multiple placeholder="请选择" class="w-280">
                    <el-option v-for="item in memberGrade" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                  </el-select>
                </template>
              </div>
            </el-form-item>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        costAllBegin: [{ validator: costAllBeginValidate, trigger: 'blur' }],
        costAllEnd: [{ validator: costAllEndValidate, trigger: 'blur' }],
        costTimeBegin: [{ validator: costTimeBeginValidate, trigger: 'blur' }],
        costTimeEnd: [{ validator: costTimeEndValidate, trigger: 'blur' }],
        avgCostBegin: [{ validator: avgCostBeginValidate, trigger: 'blur' }],
        avgCostEnd: [{ validator: avgCostEndValidate, trigger: 'blur' }],
        avgNumberBegin: [{ validator: avgNumberBeginValidate, trigger: 'blur' }],
        avgNumberEnd: [{ validator: avgNumberEndValidate, trigger: 'blur' }]
      }
    };
  },
  methods: {
    //所属开卡门店
    openCard: function() {
      this.opendCardTree = true; //显示所属开卡门店的下拉树
    },
    submit: function() {
      let data = {
