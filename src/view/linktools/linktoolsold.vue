    ...
    <el-dialog :class="activeName == 'first' ? 'min-1051' : 'min-720'" title="链接小工具" :visible.sync="mylinkToolsVisible" :before-close="handleClose">
      ...
      <div class="links-types">
        ...
        <div class="goods-category">
          <treeselect v-model="goodsCategory" :options="goodsCategoryOptions" :normalizer="normalizer" :default-expand-level="1" placeholder="所有品类" noOptionsText="暂无数据" noResultsText="无数据" @select="goodsCategorySelect" />
        </div>
      </div>
      <!-- h5 -->
      ...
      <div class="goods-links-contian">
        <div class="links-tools-row">
          <treeselect :value="category" :options="categoryOptions" :normalizer="normalizer" :default-expand-level="1" placeholder="所有品类" noOptionsText="暂无数据" noResultsText="无数据" @select="categorySelect" />

          <treeselect v-model="classify" :options="classifyOptions" :normalizer="normalizer" :default-expand-level="1" placeholder="所有分类" noOptionsText="暂无数据" noResultsText="无数据" @select="classifySelect" />
        ...
        <div class="goods-link-content">
          <ul class="goods-link-list">
            <li class="goods-link-item" v-for="(item, index) in goodsList" @click="selectLinkItem(item)" :key="index">
        ...
        <div class="links-tools-page">
          <el-pagination @size-change="linkSizeChange" @current-change="linkCurrentChange" :current-page="currentPage" :page-sizes="[20, 30, 40, 50]" :page-size="pageSize" layout="total, sizes, prev, pager, next, jumper" :total="total"> </el-pagination>
        </div>
      </div>
        ...
        <el-tab-pane label="自定义页面链接" name="fourth">
          ...
          <div class="definepage">
            <ul class="definepage-ul">
              <li class="definepage-ul-li" v-for="(item, index) in definePageList" @click="definePageClick(item)" :key="index">{{ item.name }}</li>
            </ul>
          </div>
        </el-tab-pane>
      </el-tabs>
...
export default {
  ...
  methods: {
    ...
    selectLinkItem(item) {
      ...
      log(that.selectType, selectTypeName);
      that
