<template>
  <div class="right-wrap">
                ...
                <template v-if="item.channelType == 3">
                  ...
                  <div class="form-child" v-if="item.openStatus" :key="index">
                    ...
                    <el-form-item label="适用商品" prop="name" class="padding-0">
                      ...
                      <treeselect v-if="item.channelRelationProType == 2 || item.channelRelationProType == 3" placeholder="" noOptionsText="暂无数据" noResultsText="无数据" noChildrenText="暂无数据" :multiple="true" :flat="true" :always-open="goodsOpen" :options="goodsTreeOptions" :load-options="loadOptions" :normalizer="goodNormalizer" :value-consists-of="valueConsistsOf" v-model="item.strategyGoodsRelationIdArr" /><!--:default-expand-level="1":load-options="loadOptions" :normalizer="normalizer" :always-open="goodsOpen" -->
                  ...
                  <div class="form-child margin-20" v-if="item.openStatus == true" :key="index">
                    <el-form-item label="选择店铺">
                      <el-select v-model="item.channelRelationStoreType" placeholder="请选择">
                        <el-option v-for="item in allShopOption" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                      </el-select>
                    </el-form-item>

                    <el-form-item label="天猫商品" class="padding-0">
                      <el-select v-model="item.channelRelationProType" placeholder="请选择">
                        <el-option v-for="item in allGoodOption" :key="item.value" :label="item.label" :value="item.value"> </el-option>
                      </el-select>
                      <!-- 二级 -->
                      <!--  <treeselect
    ...
    <!-- 人群筛选器的弹框 -->
    <el-dialog title="自定义人群" :visible.sync="dialogVisiblePeople" :close-on-click-modal="false" width="980px">
