      ...
      <div class="right-box">
        ...
        <div class="special-table-wrap">
          ...
          <el-table :data="tableData2" style="width: 100%">
            <el-table-column prop="enterpriseName" label="商户" sortable> </el-table-column>
            ...
            <el-table-column prop="jiesuan" label="操作">
              <template slot-scope="scope">
    ...
    <el-dialog :title="specialTitle" :visible.sync="dialogVisible" :before-close="cancelStrategyBtn" width="500px">
      <div class="special-wrap">
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="120px" class="demo-ruleForm">
          <el-form-item label="商户" prop="enterprise">
            <el-select class="change-select" v-model="ruleForm.enterprise" placeholder="选择商户">
...
export default {
  ...
  data() {
    return {
      ...
      MP_NAME: '',
      rules: {
        enterprise: [{ required: true, message: '请选择商户', trigger: 'change' }],
  ...
  methods: {
    ...
    getCostData() {
      this.loading = true;
      let para = {};
      postRequest('/api-admin/integral-costing-setting', para).then(res => {
        let data = res.data;
        this.loading = false;
        if (data.errorCode == 0) {
    ...
    saveBtn() {
      ...
      postRequest('/api-admin/save-integral-costing-setting', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
    ...
    getSpecialData() {
      let para = {};
      postRequest('/api-admin/get-special-list', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
    ...
    getChannelList() {
      let para = {};
      postRequest('/api-admin/get-channel-list', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
    ...
    getProduceList() {
      let para = {};
      postRequest('/api-admin/get-config-dict', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
    ...
    getMainList() {
      let para = {};
      postRequest('/api-admin/get-main-part-cost', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
    ...
    addStrategy(type, row) {
      ...
      if (type == 'add') {
        ...
        postRequest('/api-admin/get-special-by-id', para).then(res => {
          let data = res.data;
          if (data.errorCode == 0) {
            this.integralSettlementSettingId = data.result.integralSettlementSettingId;
            this.ruleForm.enterprise = data.result.enterpriseId;
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let obj = {
            ...
            enterpriseId: this.ruleForm.enterprise, //企业id，如果是  集团，需要传递；如果是商户，则不要该字段
          ...
          postRequest('/api-admin/save-special-integral-costing-setting', para).then(res => {
            let data = res.data;
            if (data.errorCode == 0) {
              this.dialogVisible = false;
              this.getSpecialData();
    ...
    sureDelBtn() {
      ...
      postRequest('/api-admin/delete-special-by-id', para).then(res => {
        let data = res.data;
        if (data.errorCode == 0) {
